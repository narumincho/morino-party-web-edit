...
const pointPairList: ReadonlyArray<readonly [Vec3, Vec3]> = [
  ...
  [innerN, centerO],
  //
  [innerA, xYLineSymmetry(innerA)],
  [innerB, xYLineSymmetry(innerB)],
  [innerC, xYLineSymmetry(innerC)],
  [innerD, xYLineSymmetry(innerD)],
  [innerE, xYLineSymmetry(innerE)],
  [innerG, xYLineSymmetry(innerG)],
  [centerG, xYLineSymmetry(centerG)],
  [centerH, xYLineSymmetry(centerH)],
  [centerI, xYLineSymmetry(centerI)],
  [centerJ, xYLineSymmetry(centerJ)],
  [centerK, xYLineSymmetry(centerK)],
  [centerL, xYLineSymmetry(centerL)],
  //
...
).flatMap((e) => toSymmetry(e.vec3).map((k) => ({ vec3: k, type: e.type })));
const minimumZ = positions.reduce((min, p) => (
  Math.min(min, p.vec3.z)
), positions[0]?.vec3.z!);
